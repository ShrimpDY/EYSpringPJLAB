<template>
  <div>
    <div class="QH_header_layout">
      <h3 class="QH_header">
        <span class="QH_header_span">Quarterly Key Highlights</span>
      </h3>
    </div>
    ...
    <key-quarterly-highlights ref="KeyQuarterlyHighlights"></key-quarterly-highlights>
  </div>
</template>

<script>
// import BarChart from '@/components/Cards/BarChart'
import KeyQuarterlyHighlights from '@/components/Cards/KeyQuarterlyHighlights'
...
export default {
  ...
  name: 'quarter-charts',
  components: { KeyQuarterlyHighlights },
  data: function () {
    return {
      selected: [],
      options: [],
  ...
  methods: {
    getData () {
      ...
      this.$refs.KeyQuarterlyHighlights.getDataStack('total-capital-breakdown', this.quarter, this.selected)
      this.$refs.KeyQuarterlyHighlights.getDataStack('key-exposures', this.quarter, this.selected)
      this.$refs.KeyQuarterlyHighlights.getDataStack('key-buffers', this.quarter, this.selected)
      this.$refs.KeyQuarterlyHighlights.getDataBar('total-rwa-assets', this.quarter, this.selected)
      this.$refs.KeyQuarterlyHighlights.getDataBar('key-ratios', this.quarter, this.selected)
